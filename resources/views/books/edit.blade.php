...
@section('content') <form method="POST" action="{{ route('books.update', $book) }}" enctype="multipart/form-data">
@csrf
@method('PUT')
    ...
    <div class="mb-3">
        ...
        @if(isset($categories) && $categories->count())
            ...
            <small class="d-block mt-1">Atau ubah dengan nama baru:</small>
        @endif
    ...
    <div class="mb-3">
        <label>Harga</label>
        <input class="form-control" name="price" value="{{ old('price', $book->price) }}" />
        @error('price') <small class="text-danger">{{ $message }}</small>@enderror
    </div>

    <div class="mb-3">
        <label>Stok</label>
        <input class="form-control" name="stock" value="{{ old('stock', $book->stock) }}" />
    ...
    <div class="mb-3">
        ...
        @if(!empty($book->cover_path))
            <small>Cover saat ini: <a target="_blank" href="{{ asset('storage/' . $book->cover_path) }}">lihat</a></small>
        @endif
